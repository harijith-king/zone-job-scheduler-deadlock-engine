...
def srtf():
    ...
    while len(completion_times) < len(JOBS):
        ...
        # Tie-breaking: arrival_time, then job_id.
        selected = min(
            ready_jobs,
            key=lambda job: (
...
def round_robin(quantum):
    ...
    while len(completion_times) < len(JOBS):
        ...
        while (
            ...
            ready_queue.append(arrival_order[next_arrival_index]["job_id"])
            next_arrival_index += 1

        # If queue is empty, jump to next arrival.
        if not ready_queue:
            if next_arrival_index < len(arrival_order):
    ...
    for job in JOBS:
        job_id = job["job_id"]
        turnaround_time = completion_times[job_id] - job["arrival_time"]
        waiting_time = turnaround_time - job["burst_time"]
        results[job_id] = {
            "waiting_time": waiting_time,
            "turnaround_time": turnaround_time,
...
def priority_scheduling(aging=False):
    ...
    while len(completed) < len(JOBS):
        ...
        def effective_priority(job):
            ...
            return max(1, job["priority"] - (ticks_waited // 3))
